...
int main(){
	...
	FOR(h,1,151)FOR(w,h+1,151){
		pi.PB(MP(MP(h*h+w*w,h),w));
	}
	SORT(pi);
	while(cin>>h>>w){
		if(h==0&&w==0)break;
		int tmp=h*h+w*w;
		REP(i,SZ(pi)){
			if(tmp<pi[i].first.first||(tmp==pi[i].first.first&&h<pi[i].first.second)){
				cout<<pi[i].first.second<<" "<<pi[i].second<<endl;
				break;
			}
		}
	}
