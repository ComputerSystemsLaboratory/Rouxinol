...
node *insert(node *root,int x,int isRoot)
{
	//prd(x);
	if(root==NULL)
	return newNode(x,isRoot);
	
	if(x<=root->x)
	root->left=insert(root->left,x,isRoot);
	if(x>=root->x)
	root->right=insert(root->right,x,isRoot);
