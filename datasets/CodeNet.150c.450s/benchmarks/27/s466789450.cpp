...
void print_preorder(const vector<pair<int, int> >& nodes, const vector<int>& node_val, const int r){
  if(r<0) return;
  cout << ' ' << node_val[r];
...
void print_inorder(const vector<pair<int, int> >& nodes,  const vector<int>& node_val, const int r){
  if(r<0) return;
