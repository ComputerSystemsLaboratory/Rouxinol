...
int main() {
    ...
    auto g = [](ll a, ll b) { return a + b; }; // 要素に作用素を作用
    auto h = [](ll a, ll b) { return a + b; }; // 作用素と作用素をマージ
    auto p = [](ll a, int b) { return a * b; }; // 作用素を区間加算する場合
    ll ti = 0;
    ll ei = 0;
    ...
    while (q--) {
        ...
        cin >> com >> x >> y;
        if (com) {
            cout << treeone.query(x, y + 1) << endl;
        } else {
            treeone.update(x, treeone.query(x, x + 1) + y);
        }
    }
