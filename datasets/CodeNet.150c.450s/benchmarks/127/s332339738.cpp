...
void solve() {
	...
	set<string> box;
	REP(i, S.size()) {
		string up = S.substr(0, i + 1);
		string down = S.substr(i + 1);
		string rup = up, rdown = down;
		reverse(beginend(rup));
		reverse(beginend(rdown));
		box.insert(up + down);
		box.insert(up + rdown);
		box.insert(down + up);
		box.insert(rdown + up);
		box.insert(rup + down);
		box.insert(rup + rdown);
		box.insert(down + rup);
		box.insert(rdown + rup);
	}
	out(box.size());
}
