...
int binary_search_p(int first_w, int last_w, int k, int max_w, const vector<int>&w)
{
	...
	if (first_w > last_w)for (int i = max(p_candidate, max_w);; i++)if (get_minimum_k(i) <= k)return i;
	return get_minimum_k(p_candidate) <= k ? binary_search_p(first_w, p_candidate - 1, k, max_w, w) : binary_search_p(p_candidate + 1, last_w, k, max_w, w);
}
signed main()
{
	int n = in(), k = in();
