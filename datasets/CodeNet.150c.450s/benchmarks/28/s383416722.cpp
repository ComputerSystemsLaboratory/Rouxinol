...
int binary_search_p(int first_w, int last_w, int k, int min_w, int max_w, const vector<int>&w)
{
	...
	if (min_k <= k)
	{
		return  binary_search_p(first_w, p_candidate - 1, k, min_w, max_w, w);
	}
	else if (min_k > k)
	{
		return  binary_search_p(p_candidate + 1, last_w, k, min_w, max_w, w);
	}
}
signed main()
{
	...
	cout << binary_search_p(min_w, sum_w, k, min_w, max_w, w) << endl;
}
