...
void Dijkstra(int o, int n) {
	...
	st.insert(mp(0,o));
	dist[o] = 0;
	for(int i = 0; i < n; i++) if(i != o) st.insert(mp(INF,i));
	...
	while(!st.empty()) {
		...
		if(mrc[it->s] == 0) {
			...
			for(int i = 0; i < v[it->s].size(); i++) {
				if(dist[it->s] + v[it->s][i].s < dist[v[it->s][i].f]) {
					st.erase(mp(dist[v[it->s][i].f], v[it->s][i].f));
					dist[v[it->s][i].f] = dist[it->s] + v[it->s][i].s;
					st.insert(mp(dist[v[it->s][i].f], v[it->s][i].f));
				}
			}
		}
		st.erase(it);
