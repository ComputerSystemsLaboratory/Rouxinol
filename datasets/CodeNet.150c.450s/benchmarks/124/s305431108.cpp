...
void printSPST(const vector< vector<int> >& weightedGraphMatrix) {//ShortestPathSpaningTree
	enum Color {
		WHITE, GRAY, BLACK//白は未訪問でTと隣接していない。
		//グレーはTと隣接状態だが未訪問、黒は訪問済み。木は閉路を持たないグラフなので、全域木の計算では
		//各点は1度だけ訪問されるはず。
	};
	...
	while (true) {
		...
		for (int v = 0; v != n; ++v) {
			...
			if (weightedGraphMatrix.at(u).at(v) < 0 ) continue;//辺がない場合はスキップ
			const int distanceToVThroughCurrentU = d.at(u) + weightedGraphMatrix.at(u).at(v);
			if (distanceToVThroughCurrentU >= d.at(v)) continue;
			d.at(v) = distanceToVThroughCurrentU;
			p.at(v) = u;
