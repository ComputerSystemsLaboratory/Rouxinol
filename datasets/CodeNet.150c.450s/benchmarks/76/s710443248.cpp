...
struct BinarySearchTree {
    ...
    bool find(T v, size_t x=0) {
        size_t l=nodes[x].left;
        if (~l && nodes[x].value>v)
            if (find(v, l)) return true;

        size_t r=nodes[x].right;
        if (~r && nodes[x].value<v)
            if (find(v, r)) return true;

        return nodes[x].value == v;
    }
