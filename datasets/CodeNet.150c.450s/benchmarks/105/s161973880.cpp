...
void insertVector(ll mask) {
	for (ll i = 0; i < d; i++) {
		if ((mask & (((ll)1) << i)) == 0) continue; // continue if i != f(mask)
...
bool is_present(ll v){
	ll curval = 0;
	for (ll i = 0; i < d; i++) {
		if (basis[i] && ((curval ^ v) & (((ll)1) << i))) { // If there is no basis vector with the i'th bit set, then insert this vector into the basis
			curval ^= basis[i];
		}
	}
	if(curval == v) return 1;
	else return 0;
}
