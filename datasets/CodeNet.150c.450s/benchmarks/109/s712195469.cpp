...
int main() {
  ...
  while(1){
    ...
    rep(i,n){
      string a,b;cin>>a>>b;
      ll c=0,d=0;
      c=((a[0]-'0')*10+(a[1]-'0'))*3600+((a[3]-'0')*10+(a[4]-'0'))*60+((a[6]-'0')*10+(a[7]-'0'));
      d=((b[0]-'0')*10+(b[1]-'0'))*3600+((b[3]-'0')*10+(b[4]-'0'))*60+((b[6]-'0')*10+(b[7]-'0'));
      v[c]+=1;
      v[d]-=1;
    }
    ll ans=0;
    vector<ll> sum(100001,0);
    rep(i,100000) sum[i+1]=sum[i]+v[i];
    rep(i,sum.size()) chmax(ans,sum[i]);
    cout<<ans<<endl;
  }
}
