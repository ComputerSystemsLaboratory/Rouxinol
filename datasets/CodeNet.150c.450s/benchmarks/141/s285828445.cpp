...
int side(POINT p, LINE e) {
	...
	POINT p3 = e.b; // ???????????? e ?????????
					   ...
					   // ???????????? (p2,p1), (p2,p3) ???????????? z ??????????±???????
	const double n = p1.x * (p2.y - p3.y) + p2.x * (p3.y - p1.y) + p3.x * (p1.y - p2.y);
	if (n > 0) return  1; // ???
...
signed main() {
	...
	while (std::cin >> x1 >> y1 >> x2 >> y2 >> x3 >> y3 >> xp >> yp) {
		...
		flag &= side(p1, LINE(p2, p3)) == side(pp, LINE(p2, p3));
		flag &= side(p2, LINE(p3, p1)) == side(pp, LINE(p3, p1));
		flag &= side(p3, LINE(p1, p2)) == side(pp, LINE(p1, p2));
		OUT(((flag) ? "YES" : "NO"))BR;
	}
	return 0;
}