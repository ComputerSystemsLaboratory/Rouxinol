...
void solve() {
	...
	while (1) {
		vi box(L);
		REP(i, S.size()) box[i] = S[i] - '0';
		FOR(i, S.size(), L) box[i] = 0;
		sort(beginend(box));
		auto dmin = convert(box);
		reverse(beginend(box));
		auto dmax = convert(box);
		S = to_string(dmax - dmin);
		if (calced.count(dmax - dmin)) {
			cout << calced[dmax-dmin] << " " << dmax - dmin << " " <<cnt - calced[dmax - dmin] +1 << endl;
			break;
		}
		else {
			calced[dmax - dmin] = cnt+1;
		}
		cnt++;
	}
}
