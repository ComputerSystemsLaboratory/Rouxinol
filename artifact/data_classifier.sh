...
usage() {
    echo "Usage: $0 --data <directory> --rouxinol <directory> --dataset <string> [--problems <integer>] [--samples <integer>] [--rounds <integer>] [--game <integer>] [--shuffle] [--no-cc2025]"  
    ...
    echo "  --shuffle: Shuffle problems and samples (Default is false)."  
...
OPTIONS=$(getopt -o "" --long data:,rouxinol:,dataset:,problems:,samples:,rounds:,game:,shuffle,no-cc2025 -- "$@")  
if [ $? -ne 0 ]; then  
    usage  
fi  
        ...
        for (( tr=0; tr<${#train[@]}; tr++ )); do  
            ...
            for (( te=0; te<${#test[@]}; te++ )); do  
                ...
                if [ "${representation}" == "programl" ]; then
                    python ${rouxinol_directory}/examples/classification_programl.py \
                    ...
                    python ${rouxinol_directory}/examples/classification.py \
                        --data_directory=${data_directory} \
                        --dataset_name=${dataset} \
                        --train=${train_label} \
                        --test=${test_label} \
