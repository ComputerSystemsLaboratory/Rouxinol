...
flags.DEFINE_string('plot_name', 'elapsed', 'Name of the plot file') 

flags.DEFINE_list('flags', None, 'List of flags to process') 
flags.DEFINE_list('representations', None, 'A list of representation to process')   
...
def create_stats_dict(data_directory, representations, flags, suffix=None):  
	"""  
	...
	for representation in representations:  
		...
		for flag in flags:  
			...
			ir_file = os.path.join(data_directory, flag, "stats", f"ir_elapsed_{file_suffix}.yml")  
			exe_file = os.path.join(data_directory, flag, "stats", f"exe_elapsed_{file_suffix}.yml")  
			rep_file = os.path.join(data_directory, flag, "stats", f"rep_elapsed_{file_suffix}.yml")  
...
def save_stats_dict(stats_dict, data_directory):  
    ...
    """  
    output_file = os.path.join(data_directory, "stats", "elapsed.yml")  
...
def create_bar_plot(stats_dict, representations, flags, data_directory, plot_name):  
	...
	# Save the plot  
	plot_path = os.path.join(data_directory, "stats", f"{plot_name}.pdf")  
...
def main(argv):  
	...
	output_file = os.path.join(FLAGS.data_directory, "stats", "elapsed.yml")
	if os.path.isfile(output_file):
		with open(output_file, "r") as file:  
			stats_dict = yaml.safe_load(file) 
	else:
		# Create the stats_dict and save it to the YAML file  
		stats_dict = create_stats_dict(FLAGS.data_directory, FLAGS.representations, FLAGS.flags, FLAGS.suffix)  
		save_stats_dict(stats_dict, FLAGS.data_directory)  

	# Create the bar plot and save it to the data_directory
	create_bar_plot(stats_dict, FLAGS.representations, FLAGS.flags, FLAGS.data_directory, FLAGS.plot_name)  
