...
class LLVMInstsCounterVisitor(Visitor):
    def __init__(self):
        Visitor.__init__(self)
        self.D = {}

    def visit(self, v):
        if isinstance(v, llvm.insts.FunctionInfo):
            self.D[v.name] = v.instructions


class LLVMInstsCounterBuilder(common.RepresentationBuilder):
    ...
    def string_to_info(self, src, *args, **kwargs):
        ...
        with clang_driver_scoped_options(self.__clang_driver, additional_include_dir=additional_include_dir, filename=filename):
            info = self.__extractor.InstsFromString(src_bytes)
    ...
    def info_to_representation(self, info, visitor=LLVMInstsCounterVisitor):
        # Start timing
        runtime = {
            'elapsed_time': 0.0,
            'user_time': 0.0,
