...
class LLVMMilepostVisitor(Visitor):
    ...
    def visit(self, v):
        if isinstance(v, llvm.msf.FunctionInfo):
            self.D[v.name] = v.features


class LLVMMilepostBuilder(common.RepresentationBuilder):
    ...
    def string_to_info(self, src, *args, **kwargs):
        ...
        with clang_driver_scoped_options(self.__clang_driver, additional_include_dir=additional_include_dir, filename=filename):
            info = self.__extractor.MilepostFromString(src_bytes)
    ...
    def info_to_representation(self, info, visitor=LLVMMilepostVisitor):
        # Start timing
        runtime = {
            'elapsed_time': 0.0,
            'user_time': 0.0,
