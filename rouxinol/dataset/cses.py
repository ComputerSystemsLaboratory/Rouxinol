...
class CSES(Dataset):
    ...
    def _preprocess_sample(
        ...
        label = "test" if "/test/" in sample else "train"
        ...
        if representation == "cfggrindHistogram":
            if cfggrind_static:
                data.append({"x": embeddings[0], "y": sample_class, "info": label})
            else:
                data.append({"x": embeddings[1], "y": sample_class, "info": label})
        else:
            data.append({"x": embeddings, "y": sample_class, "info": label})
    ...
    def select_problems_and_samples(
        ...
        **kwargs
        ...
        for problem in problem_names:
            data[problem] =  multiple_file_types(os.path.join(self.content_dir, problem, "test"), "*.cpp")
            data[problem].extend(multiple_file_types(os.path.join(self.content_dir, problem, "train"), "*.cpp"))
