...
def samples_to_rep(args: Tuple) -> List:
    ...
    for idx, X_sample in enumerate(X_samples):
        ...
        if is_cfggrind_histogram:
            representation_filename = f"{rep_dir}/{filename}_{cfggrind_representation}.yl"
        else:
            representation_filename = f"{rep_dir}/{filename}.yl"

        if not os.path.isfile(representation_filename):
            continue

        with open(representation_filename, "r") as fin:
            representation = yl.load(fin)
