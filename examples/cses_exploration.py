...
from rouxinol.utility import create_timestamped_directory, get_next_filename, split_train_test_data
...
flags.DEFINE_string("label_dir", "O0", "Directory to store the data.")    
flags.DEFINE_string("workload", "workload.yml", "Workload description filename.")  

flags.DEFINE_integer("timeout", 240, "Timeout in seconds")
flags.DEFINE_integer("problems", 32, "Number of classes")
flags.DEFINE_integer("samples", 100, "Number of samples per class")
flags.DEFINE_integer("workers", 10, "Number of worker processes")
flags.DEFINE_integer("entry", 1, "Number of the workload")
...
flags.DEFINE_enum("representation", "llvmHistogram", ["llvmHistogram", "x86Histogram", "cfggrindDynamicHistogram", "cfggrindHybridHistogram", "performanceCounterHistogram", "ir2vec", "inst2vecPreprocessed", "inst2vecEmbeddings"], "Representations to extract")
...
def main(argv):
    ...
    workload_filename = os.path.join(
                            data_directory,
                            FLAGS.label_dir,
                            "stats",
                            FLAGS.workload
    ...
    if FLAGS.force or not os.path.isfile(workload_filename):
        ...
        with open(workload_filename, "r") as fout:
    ...
    ir_filename = get_next_filename(
                        os.path.join(data_directory, FLAGS.label_dir, "stats", f"ir_elapsed_{FLAGS.representation}"),
                        "yml"
                    )
    exe_filename = get_next_filename(
                        os.path.join(data_directory, FLAGS.label_dir, "stats", f"exe_elapsed_{FLAGS.representation}"),
                        "yml"
                    )
    repr_filename = get_next_filename(
                        os.path.join(data_directory, FLAGS.label_dir, "stats", f"rep_elapsed_{FLAGS.representation}"),
                        "yml"
                    )
    ...
    model_transform = {
        ...
        "cfggrindDynamicHistogram": (RandomForestModel, transform_split_data_with_dict),
        "cfggrindHybridHistogram": (RandomForestModel, transform_split_data_with_dict),
        "performanceCounterHistogram": (RandomForestModel, transform_split_data_with_dict),
        "inst2vecPreprocessed": (RandomForestModel, transform_split_data_with_list),
        "inst2vecEmbeddings": (RandomForestModel, transform_split_data_with_list),
    ...
    stats_filename = get_next_filename(
                        os.path.join(FLAGS.data_directory, "stats", f"stats_{FLAGS.representation}_{FLAGS.label_dir}_{FLAGS.label_dir}_{dataset_name[:-4]}_{FLAGS.problems}_{FLAGS.samples[0]}"),
                        "yml"
                )
