...
usage() {  
    echo "Usage: $0 -a STR -b LIST -d LIST -i INT -l DIR -p DIR -t LIST -u LIST [-f]"  
    echo ""  
    echo "Commands:"
...
OPTIONS=$(getopt -o p:b:t:d:i:a:l:u:f --long package:,benchmarks:,tasklets:,dpus:,timeout:,passes:,log:,upmem:,force -- "$@")  
if [ $? -ne 0 ]; then  
    usage  
fi  
