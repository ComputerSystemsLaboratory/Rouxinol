...
from rouxinol.utility import train_test_data_with_dict, train_test_data_with_list
...
from utility import parse_log_file, load_offline_embedding, process_data
...
def main(
    ...
    data = load_offline_embedding(FLAGS.data_directory, FLAGS.benchmarks, FLAGS.baseline, FLAGS.target)

    # Split the data
    if FLAGS.model == "L2P":
        data_fine_tuning, data_prediction = train_test_data_with_dict(
                                                data, 
                                                test_ratio=1.0-FLAGS.fine_tuning_size, 
                                                transformer=FLAGS.transformation, 
                                                regression=True
                                            )
        data_fine_tuning, data_val_fine_tuning = train_test_data_with_list(
                                                    data_fine_tuning, 
