...
usage() {  
    echo "Usage: $0 -a STR -b LIST -d LIST -l DIR -p DIR -t LIST -u LIST [-f]"  
    echo ""  
    echo "Commands:"
...
OPTIONS=$(getopt -o p:b:t:d:a:l:u:f --long package:,benchmarks:,tasklets:,dpus:,passes:,log:,upmem:,force -- "$@")  
if [ $? -ne 0 ]; then  
    usage  
fi  
